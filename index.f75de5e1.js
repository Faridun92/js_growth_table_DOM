...
container.addEventListener("click", function(e) {
    ...
    if (e.target.matches(".append-row.button")) {
        if (rows < 10) {
            ...
            for(let i = 0; i < columns; i++)newRow.insertCell(i).textContent = "";
            rows += 1;
        }
    }
    ...
    if (e.target.matches(".append-column.button")) {
        if (columns < 10) {
            for(let i = 0; i < rows; i++)table.rows[i].insertCell().textContent = "";
            columns += 1;
        }
    }
    if (e.target.matches(".remove-column.button")) {
        if (columns > 2) {
            for(let i = 0; i < rows; i++)table.rows[i].deleteCell(-1);
            columns -= 1;
        }
    }
    toggleButtons();
